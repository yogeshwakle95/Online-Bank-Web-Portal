...
import "./AdminWithdrawlRequest.css";
...
function AdminWithdrawlRequest() {
...
const[userAccountData,setUserAccountData]= useState([])
  ...
  useEffect(() => {
    axios.get("http://localhost:4000/api/userTransaction/adminUserTransaction/data").then((res)=>setData(res.data?.filter((z)=>z.transactionType == "Withdrawl")))
    }, [])
 console.log(data);
    const fun=async(e)=>{
...
if(e.isApproved == "cancel"){
 ...
 await axios.put(`http://localhost:4000/api/userTransaction/${e.userTransactionId}`,{isApproved:"cancel"}).then((res)=>addToast("Rejected",{appearance:"error"}))
 await axios.get("http://localhost:4000/api/userTransaction/adminUserTransaction/data").then((res)=>setData(res.data))
}else{
  await  axios.put(`http://localhost:4000/api/userAccount/update/${e?.id+"-W"}`,update).then((res)=>(setError(res.data),error =(res.data.message)))
  
  setTimeout(()=>{
    if(error == "Not enough Funds"){
      approvedStatus = {
        isApproved :"insufficient"
      }
     
      axios.put(`http://localhost:4000/api/userTransaction/${e.userTransactionId}`,approvedStatus).then((res)=>addToast("Approved",{appearance:"success"}))
...
axios.get("http://localhost:4000/api/userTransaction/adminUserTransaction/data").then((res)=>setData(res.data))
    ...
    const final_data = data?.filter((y)=>y.isApproved == "false").filter((z)=>z.transactionType == "Withdrawl")
...
<div className='nit1'>Withdrawal Request</div>
