...
import './WithdrawlTransactionHistory.css'
...
useEffect(()=>{
    axios.get(`http://localhost:4000/api/userTransaction/userTransactionAll/data/${id}`).then((res)=>setData(res.data?.filter((e)=>e.transactionType == "Withdrawl")))
},[])
  ...
  return (
    <div>
        ...
        <div className="container">
          ...
          <div className='iit2_btn'><Link className='iit2_btn_link' to={"/withdrawl"}><i class="fa fa-arrow-left" id='iit2_icon' ></i> Back</Link></div>
          <div className='iit1'>Withdrawal Transaction History</div>
    ...
    <tr key={j}>
      ...
      <td>
        {e.isApproved === "false" ? (
          ...
          <div style={{ color: "blue" }}>Insufficient Balance</div>
        ) : e.isApproved === "cancel" ? (
